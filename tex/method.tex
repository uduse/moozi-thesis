...
The third key feature of \moozi is efficient acting with interdependent inferences.
...
\includeimage[1]{stacking}{
    ...
    \textit{Actions to Planes}: One-hot encodes actions into feature planes that have the same resolution (i.e., same width and height) as the observations, $|\mathcal{A}^a| = 2$.
...
\subsection{The \moozi Neural Network} \label{sec:nn}
...
\note{New: Handling two players in the planner.}

The planner component $\mathcal{P}$ takes a stacked history as its input (see Section \ref{sec:history_stacking}), performs a search, collects search statistics, and outputs a action and search statistics
\begin{align*}
...
In both single-player and two-player environments, we train the neural network from the perspective of the first player.
...
\note{New: Add figures in Section \ref{sec:logging}}

\includeimage[0.7]{tb_2}{\textbf{\moozi \textit{Tensorboard} dashboard.}}{}

\includeimage{sprite_trimmed}{\textbf{
        \moozi produces trajectories in \textit{Asterix} as GIFs with annotations, presented here as tiled images of every four frames.
    }}{
    In these images, \textit{R} is the reward $r_{t-1}$ given from the last timestep,
    ...
    and \textit{A} is the index of the action to take.
}
