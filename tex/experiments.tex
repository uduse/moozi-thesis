...
\section{Improving Sample Efficiency with Reanalyze} \label{sec:exp:re}
...
\section{\moozi Learning through Self-play in Two-players Game \textit{Breakthrough}} \label{sec:exp:breakthrough}
\includeimage[0.8]{breakthrough}{
...
\includeimage[0.8]{breakthrough_elo}{\textbf{
...
\includeimage{half_tree}{\textbf{\moozi planning with a learned model.}}
...
% The light pink nodes are \textit{delusions} created by taking an illegal action from a legal state.
% The white nodes are deep \textit{delusions} created by taking any action from an illegal state.
% One special case of a \textit{delusion} is when the model takes the dummy action after a terminal state.
% Even though the dummy action is not a legal action in the game, it represents a valid prediction that the game will end.

\includeimage{full_tree}{\textbf{MooZi planning with a learned model.}}{
    The model accurately rollout game states from the starting state \textit{A} all the way to the terminal state \textit{B}.
